...
// ── CHANGE 1: Flight category added between Travel and Other ──
const CATEGORY_META = {
  ...
  return (
    <div style={S.tripShell}>
      ...
      <div style={{ ...S.tabContent, position: "relative" }}>
        ...
        {activeTab === "expenses"  && <ExpensesTab  trip={trip} onModal={onModal} expRefresh={itinRefresh} profile={profile} user={user} expenses={expenses} settlements={settlements} myName={myName} onEditExpense={setEditingExpense} />}
        ...
        {modal === "settle"        && <SettleModal settlements={settlements} myName={myName} trip={trip} onClose={() => setModal(null)} user={user} />}
...
function ExpensesTab({ trip, onModal, expRefresh, profile, user, expenses, settlements, myName, onEditExpense }) {
  const [filter, setFilter] = useState("All");
  const [memberCount, setMemberCount] = useState(0);
  // ── CHANGE 1 cont: Flight added to filter cats ──
  const cats = ["All","Dining","Drinks","Stay","Activity","Shopping","Travel","Flight","Other"];
...
function AddExpenseModal({ onClose, trip, onAdd, user, profile, existingExpense }) {
    ...
    const fetchMembers = async () => {
      ...
      const others = (memberRows || [])
        ...
        .filter(n => {
          ...
          if (/^[a-z]+\.[a-z]+\d+$/i.test(n)) return false;
          if (seen.has(lc)) return false;
          seen.add(lc);
          return true;
        });

      const sorted = [userDisplay, ...others];
      setMembers(sorted);
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, background: P.phoneBg, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* ── Step 1 ── */}
      {step === 1 && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "0 22px", justifyContent: "space-between" }}>
          <div>
            {/* Category */}
            ...
            <div style={{ textAlign: "center", padding: "2px 0 4px", borderBottom: `1px solid ${P.surface3}`, marginBottom: 6 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: P.textMuted, letterSpacing: "2px", marginBottom: 0 }}>AMOUNT</div>
              ...
              {perPerson && <div style={{ fontSize: 11, color: P.slateBlue, marginTop: 0 }}>${perPerson}/person</div>}
            </div>

            {/* Description */}
            <div style={{ marginBottom: 6 }}>
              <div style={S.fieldLbl}>DESCRIPTION</div>
              <input style={{ ...S.input, padding: "10px 16px" }} placeholder="e.g. Dinner at Coco's"
...
// ─── DATE TIME PICKER (CHANGE 2: desktop-compatible, no opacity:0 overlay) ───
...
// ── CHANGE 2: Replaced opacity:0 overlay approach with direct styled native inputs ──
        ...
        <div style={{ position: "relative" }}>
          <input
            ...
            style={{
              ...S.input,
              colorScheme: "dark",
              color: day ? P.terracotta : P.textMuted,
              fontWeight: day ? 700 : 400,
              fontSize: 14,
              paddingRight: day ? 36 : 16,
              cursor: "pointer",
              width: "100%",
        ...
        <div style={{ position: "relative" }}>
          <input
            ...
            style={{
              ...S.input,
              colorScheme: "dark",
              color: time ? P.terracotta : P.textMuted,
              fontWeight: time ? 700 : 400,
              fontSize: 14,
              paddingRight: time ? 36 : 16,
              cursor: "pointer",
              width: "100%",
              boxSizing: "border-box",
            }}
...
function AddItinModal({ onClose, trip, onAdd }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 100, background: P.phoneBg, display: "flex", flexDirection: "column" }}>
      ...
      {/* Content — no scroll */}
      <div style={{ flex: 1, padding: "0 22px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <div>
          {/* Type */}
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <input ref={detailRef} style={{ ...S.input, fontSize: 14, padding: "10px 16px" }} placeholder="Confirmation code, address, notes..." defaultValue="" />
          </div>
          {/* Date + Time */}
...
function SettleModal({ settlements, myName, trip, onClose, user }) {
  const [marked, setMarked] = useState([]);
  const [payHandles, setPayHandles] = useState({}); // keyed by display_name
  const [copied, setCopied] = useState(null);

  useEffect(() => {
    const fetchHandles = async () => {
      // Get all trip_members with user_ids, then fetch their profiles + payment handles
      ...
      const map = {};
      (profiles || []).forEach(p => {
        if (p.display_name) map[p.display_name] = p;
      });
      setPayHandles(map);
    };
    fetchHandles();
  ...
  const PayButtons = ({ toName, amount, settlementKey }) => {
    ...
    const hasVenmo = p.venmo_handle;
    const hasCashApp = p.cashapp_handle;
    const hasZelle = p.zelle_handle;
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        ...
        <a
          href={hasVenmo
            ...
            : `https://venmo.com/`}
          target={hasVenmo ? undefined : "_blank"}
          rel="noopener noreferrer"
          style={{ ...S.payBtn, textDecoration: "none", display: "flex", alignItems: "center", gap: 4,
            ...(hasVenmo ? { borderColor: "#3D95CE50", color: "#3D95CE" } : {}) }}>
          {hasVenmo ? "💸" : ""} Venmo
        </a>
        {/* Cash App */}
        <a
          ...
          rel="noopener noreferrer"
          style={{ ...S.payBtn, textDecoration: "none", display: "flex", alignItems: "center", gap: 4,
            ...(hasCashApp ? { borderColor: "#00D64F50", color: "#00D64F" } : {}) }}>
          {hasCashApp ? "💸" : ""} Cash App
        </a>
        {/* Zelle — copy to clipboard since no universal deep link */}
        <button
          onClick={() => hasZelle ? copyZelle(p.zelle_handle, zelleKey) : window.open('https://www.zellepay.com/', '_blank')}
          style={{ ...S.payBtn,
            ...(hasZelle ? { borderColor: "#6D1ED450", color: "#9B59D4" } : {}),
            ...(copied === zelleKey ? { borderColor: P.success + "60", color: P.success } : {}) }}>
          {copied === zelleKey ? "✓ Copied" : (hasZelle ? "📋 Zelle" : "Zelle")}
        </button>
      </div>
    );
  };
              ...
              {mine.map((s, i) => (
                <div key={i} style={{ ...S.settleRow, opacity: marked.includes(`m${i}`) ? 0.4 : 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    ...
                    <button onClick={() => toggle(`m${i}`)} style={{ ...S.markBtn, ...(marked.includes(`m${i}`) ? S.markBtnDone : {}), flexShrink: 0 }}>
