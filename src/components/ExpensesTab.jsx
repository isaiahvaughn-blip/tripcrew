...
export function AddExpenseModal({ onClose, trip, onAdd, user, profile, existingExpense }) {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      const { data: memberRows } = await supabase.from("members").select("name").eq("trip_id", trip.id);
      const userDisplay = profile?.display_name || user?.email?.split("@")[0] || "Me";
      const seen = new Set([userDisplay.toLowerCase()]);
      const others = (memberRows || []).map(m => m.name).filter(n => {
      ...
      setMembers(sorted);
      if (!existingExpense) setExp(e => ({ ...e, paidBy: userDisplay, splitWith: sorted }));
      else if (!exp.paidBy)  setExp(e => ({ ...e, paidBy: sorted[0] || userDisplay }));
    };
    fetchMembers();
  }, [trip.id]);
...
export default function ExpensesTab({ trip, onModal, profile, user, expenses, settlements, myName, onEditExpense }) {
    ...
    <div style={S.tabScroll}>
      <div style={S.tabTopRow}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {settlements.length > 0 && <button style={{ ...S.actionBtn, borderColor: P.lightBlue + "60", color: P.lightBlue }} onClick={() => onModal("settle")}>⚖️ Settle</button>}
